...
1. **Read the candidates in order** using `mcp__claude_ai_Google_Drive__read_file_content`, starting with the first. The document has two sections: a Gemini-generated summary ("Resumen" / "Summary") — ignore this — and a verbatim transcript with speaker labels and timestamps — use only this.
   ...
   - **If a speaker is labelled by conference room, not by name** (`MLC-Costanera-38-…`, `MLA-PoloDot-9-…`): people dialing in from a room device are attributed to the room resource, so those lines arrive anonymous. Resolve them against the attendee list and the conversational context — who is addressed by name, who answers for which team, who owns the artifact on screen. **A single room label is often shared by two or more people**; split it by context rather than collapsing it to one speaker. State in the return to the main agent which labels were resolved and to whom, so the user can confirm — those attributions are inference, not transcript fact, and they are the one part of the note a reader cannot check.
